...
int main(){
	...
	while(file.good()){
        ...
        if(username== "")
            break;
		UserList userlist;
		user* temp = userlist.createNewUserNode(username, password, email);
		userlist.InsertIntoStartofUserList(temp);		
	}
    file.close();
	...
	while(true){
		...
		if (option == 1){
			...
			user* usernode = userlist.createNewUserNode(username, password, email);
			userlist.InsertIntoStartofUserList (usernode);
